...
final class IceBarPanel: NSPanel {
    ...
    /// Manager for the Edward Bar's color.
    ...
    /// Creates a new Edward Bar panel.
    init() {
        super.init(
            contentRect: .zero,
...
private struct IceBarContentView: View {
    ...
    private var content: some View {
        if !ScreenCapture.cachedCheckPermissions() {
            HStack {
                Text("The Edward Bar requires screen recording permissions.")
            ...
            Text("Edward cannot display menu bar items for automatically hidden menu bars")
                .padding(.horizontal, 10)
